...
int main() {
    cv::Mat image = cv::imread("test5.bmp");
    ...
    if (weightedAvgPos.x >= 0 && weightedAvgPos.y >= 0) {
        cv::circle(image, weightedAvgPos, 2, cv::Scalar(0, 255, 0), -1); 
        cv::line(image, cv::Point(weightedAvgPos.x - 10, weightedAvgPos.y), cv::Point(weightedAvgPos.x + 10, weightedAvgPos.y), cv::Scalar(0, 255, 0), 2);
        cv::line(image, cv::Point(weightedAvgPos.x, weightedAvgPos.y - 10), cv::Point(weightedAvgPos.x, weightedAvgPos.y + 10), cv::Scalar(0, 255, 0), 2);

        cv::imwrite("output6.bmp", image);
    }
    return 0;
}